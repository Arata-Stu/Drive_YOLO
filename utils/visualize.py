...
def visualize_sequence_from_dataset_numpy(dataset, frame_indices: list, mode='val', cols=4):
    """
    データセットから指定されたフレームを可視化し、クラスごとに異なる色で描画します。

    Args:
        dataset: 可視化するデータセット。各フレームはnumpyベース。
    ...
    for i, idx in enumerate(frame_indices):
        data = dataset[idx]
        events = data['images']  # (ch, h, w) numpy形式
        labels = data['labels']  # numpy形式のラベル
        
        # フレーム変換と描画
        frame = events  # (ch, h, w)
        img_uint = np.transpose(frame, (1, 2, 0)).astype('uint8').copy()

        # バウンディングボックスの描画
        for bbox in labels:
            if np.all(bbox == 0):  # 無効なバウンディングボックスはスキップ
                continue
...
def visualize_sequence_from_dataset_tensor(dataset, frame_indices: list, mode='train', cols=4):
    """
    データセットから指定されたフレームを可視化し、クラスごとに異なる色で描画します。
